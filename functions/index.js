...
app.post("/createStripeCheckout", (req, res) => {
  cors(req, res, async ()=>{
    try {
      ...
      res.send({clientSecret: null, sessioned: null, error: error});
    }
  });
});

let whSec;
whSec = `${process.env.REACT_APP_WEBHOOK_SIGN_SEC}`;
...
app.post("/createAccountLink", (req, res) => {
  cors(req, res, async ()=>{
    try {
...
app.post("/payCompetitionCharge", (req, res) => {
  cors(req, res, async ()=>{
    try {
...
app.post("/createTransferToWinner", (req, res) => {
  cors(req, res, async () => {
    try {
      const {
        destinationId,
...
app.post("/webhook", bodyParser.raw({type: "*/*"}), (req, res) => {
  cors(req, res, async ()=>{
    ...
    try {
      ...
      res.status(400).send(`Webhook failed ${error}`).end();
    }
  });
});
