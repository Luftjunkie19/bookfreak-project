...
function DefaultRightBar({ }: Props) {
    ...
    return (
        <div className={` ${!user || location.includes('/search') || location.includes('/competition/') || location.includes('/club') || location.includes('/signup') || location.includes('/login') || location.includes('form/') || location.includes('/chat') ? 'hidden' : 'sm:hidden lg:flex'} h-screen ${location.includes('/profile/') && 'sm:hidden xl:flex'} min-w-32 lg:max-w-48 2xl:max-w-64 w-full flex flex-col gap-2 border-l-2 border-dark-gray`}>
