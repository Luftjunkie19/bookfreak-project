...
function DefaultLeftBar({ }: Props) {
  ...
  return (
    <div className={`h-screen overflow-y-auto ${location.includes('/search') || location.includes('/competition/') || location.includes('/club/') || location.includes('form/test') || location.includes('/signup') || location.includes('/login') || location.includes('/profile/') ? 'hidden': 'sm:hidden lg:flex'} z-40 py-4 px-2 lg:max-w-fit xl:max-w-36 2xl:max-w-64 w-full  border-r-dark-gray  flex-col gap-2 rounded-r-xl border-r-2 `}>          
