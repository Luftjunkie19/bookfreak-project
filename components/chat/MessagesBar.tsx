...
const chatMessages=chatMessagesObjects.map((obj:any[]) => {
                  ...
                  >
                      ...
                      <p className="font-semibold">
                        {
                          users
                            .filter(
                              (user, i) =>
                                user.id === (entitledToChat[i] as any).entitledUserId &&
                                (entitledToChat[i] as any).entitledChatId === doc.chatId
                            )
                            .find((chatter) => chatter.id !== user.uid)
                            ?.nickname
                        }
                    ...
                    <div className="w-full flex justify-between flex-wrap p-1 gap-2">
                      <p className="flex items-center">
                        {chatMessages.filter(
                          (message:any) => message.chatId === doc.chatId
                        ).length > 0 &&
                          users.find(
                            (chatter:any) =>
                              chatter.id ===
